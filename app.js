...
function init() {
    selectVilla('oh-yeah');
    updateStats();
    document.querySelector('.category-btn').classList.add('active'); // Default active category
}
...
function renderInventory(category = 'all') {
    ...
    // Skip the first blank item for display
    filteredItems.slice(1).forEach(item => {
    ...
    document.querySelector(`.category-btn[onclick="renderInventory('${category}')"]`).classList.add('active');
}

function handleItemClick(item) {
    if (item.name === 'Wellness balíček') {
        const price = prompt('Zadejte cenu wellness balíčku v EUR:');
